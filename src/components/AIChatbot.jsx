...
export default function AIChatbot() {
  ...
  const speak = (text) => {
    ...
    if (synth) {
      ...
      // Enhanced male voice search
      const maleVoice = voices.find(v => v.lang.startsWith(lang) && (v.name.toLowerCase().includes('male') || v.name.toLowerCase().includes('david') || v.name.toLowerCase().includes('mark') || v.name.toLowerCase().includes('google نايف'))) 
          ...
          <div className="p-4 border-t border-gray-100 dark:border-gray-800 bg-white/50 dark:bg-gray-900/50 backdrop-blur-md">
            <div className="flex gap-2 items-center bg-gray-100 dark:bg-gray-800 p-1.5 rounded-[2rem] border border-gray-200/50 dark:border-white/5 shadow-inner">
              ...
              <input 
                ...
                placeholder={isRTL ? "أؤمرني يا وحش..." : "Command me boss..."}
